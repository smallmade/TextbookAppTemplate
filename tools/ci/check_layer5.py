...
one is handed a malformed fixture per rule and must reject every one.
"""
...
def check(rows: list[dict[str, str]], source: str, name: str) -> list[str]:
    ...
    for row in rows:
        ...
        # before location/numbers, because `unavailable` (L5-7) has neither
        # and branches away from every check below this point.
...
def self_test() -> int:
    """Hand the gate one known-bad fixture per rule, and the good forms."""
